...
## ⚡ MUDANÇA IMPORTANTE (23/06/2026) — Fonte única `data.js`

**A partir de agora, a atualização mensal de POSIÇÕES é feita editando UM arquivo só: `data.js`** (`window.BAROLO_DATA`).

As 6 páginas HTML (`portfolio_analytics`, `index`, `relatorio`, `pools`, `ferramentas`, `emprestimos`) carregam o `data.js` via `<script src="data.js">` e leem qty/invested/dívidas/APYs dele — com os valores hardcoded antigos só como **fallback** (se o `data.js` falhar). Não precisa mais editar os 6 arquivos para mudar posições.

**O que atualizar no `data.js`:** `holdings[]` (qty + invested), `stables[]`, `defi.aave/kamino` (supply/borrow qty + apy), `debt`, `stablesTotalUSD`. Tudo num lugar só.

**`invested` canônico = USD realmente pago** (decisão de 23/06/2026). NÃO usar o "custo" derivado do CoinGecko (`valor − P&L`), que tem artefato em stablecoins (mostra USDT com "lucro").

As seções abaixo (mapeamento print→código dos 6 arquivos) viram **referência/fallback** — o caminho principal agora é o `data.js`. Depois de editar o `data.js`, criar o snapshot JSON em `EXPORTS SEMANAIS/` como antes.
